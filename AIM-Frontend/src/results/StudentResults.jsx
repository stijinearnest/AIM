...
export default function StudentResults() {
  ...
  const allLoadedStudentsHaveRequiredResults =
    students.length > 0 &&
    students.every(
      (student) => String(student.ogpa).trim() !== ""
    );
  ...
  const saveResults = async () => {
    if (!allLoadedStudentsHaveRequiredResults) {
      showToast("Enter OGPA for every loaded student before saving", "error");
      return;
    }
    ...
    try {
      ...
      const payload = {
        ...
        results: students.map((student) => ({
          ...
          ogpa: student.ogpa,
          marks: String(student.marks).trim() === "" ? null : student.marks,
        })),
      };
  ...
  const saveEditedResult = async () => {
    ...
    try {
      ...
      await apiPut(`/rank/result/edit/${editingStudent.student_id}/`, {
        ...
        ogpa: editForm.ogpa,
        marks: String(editForm.marks).trim() === "" ? null : editForm.marks,
                  ...
                  <div style={styles.studentTableWrapper}>
                    ...
                    <div style={styles.modalFooter}>
                      <div style={styles.saveHelpText}>
                        OGPA is required for every loaded student before saving.
                      </div>
                      <button
